...
public class DiffMatchPatch {
  ...
  private LinkedList<Diff> diff_compute(String text1, String text2,
    ...
    if (hm != null) {
      ...
      LinkedList<Diff> diffs_a = diff_main(text1_a, text2_a,
                                           checklines, deadline);
      Collection<Diff> diffs_b = diff_main(text1_b, text2_b,
              checklines, deadline);
      // Merge the results.
      diffs = diffs_a;
  ...
  private LinkedList<Diff> diff_bisectSplit(String text1, String text2,
    ...
    Collection<Diff> diffsb = diff_main(text1b, text2b, false, deadline);
   ...
   * @param lineArray List of unique strings.
   */
  protected void diff_charsToLines(Iterable<Diff> diffs, List<String> lineArray) {
    StringBuilder text;
    for (Diff diff : diffs) {
   ...
   * @param diffs LinkedList of Diff objects.
   */
  public void diff_cleanupSemanticLossless(AbstractList<Diff> diffs) {
   ...
   */
  public int diff_xIndex(Iterable<Diff> diffs, int loc) {
    int chars1 = 0;
    int chars2 = 0;
    int last_chars1 = 0;
    int last_chars2 = 0;
   ...
   */
  public String diff_prettyHtml(Iterable<Diff> diffs) {
   ...
   * @return Source text.
   */
  public String diff_text1(Iterable<Diff> diffs) {
   ...
   */
  public String diff_text2(Iterable<Diff> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff aDiff : diffs) {
      if (aDiff.operation != Operation.DELETE) {
        text.append(aDiff.text);
   ...
   */
  public int diff_levenshtein(Iterable<Diff> diffs) {
    int levenshtein = 0;
    int insertions = 0;
    int deletions = 0;
    for (Diff aDiff : diffs) {
   ...
   * @return Delta text.
   */
  public String diff_toDelta(Iterable<Diff> diffs) {
    StringBuilder text = new StringBuilder();
    for (Diff aDiff : diffs) {
      switch (aDiff.operation) {
      case INSERT:
   ...
   * @return LinkedList of Patch objects.
   */
  public LinkedList<Patch> patch_make(String text1, Deque<Diff> diffs) {
   ...
   */
  public LinkedList<Patch> patch_deepCopy(Iterable<Patch> patches) {
   ...
   */
  public void patch_splitMax(AbstractList<Patch> patches) {
  ...
  public static class Diff {
    ...
    public boolean equals(Object d) {
      try {
        return (((Diff) d).operation == operation)
               && (((Diff) d).text.equals(text));
